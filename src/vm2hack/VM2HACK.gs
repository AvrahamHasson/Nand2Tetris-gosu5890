...
class VM2HACK {
  ...
  public function vm2hack(path:String) : String {
    ...
    var returnAdd=0
    var fileName=""
    //If there are more than 1 .vm file - then bootstrap!
    if(myVMdirec.vmFiles().size()>1){
      ASMcontent += "@256\n" + "D=A\n" + "@SP\n" + "M=D\n" +
      call_command("call Sys.init 0",returnAdd)
      returnAdd++
    }
    //Ignore lines like //fname.vm
    for (var line in VMcontent) {
      if(line.startsWith("//")){
        ASMcontent+=line+"\n"
        fileName=line.substring(2, line.length() - 3) //For example "//fname.vm" -> "fname"
      }
      //Else it's a command
      else {
  ...
  //Binary operations
  //Helping binary operations, every binary operation has the same code structure except the operation itself.
  ...
  private function add_command():String { return "// vm command: add\n" + binaryOpH("+") }
  ...
  //Comparison
  //Helps comparison operations, each comparison operation has the same code structure except the operation itself.
  ...
  //Pushes D and up the stack by one
  var pushH= "@SP\n" +//A=0
      "A=M\n" +//A=270
